...
The Jev mode requires a separate approval record, a TypeSafe API key, an explicit call cap, an authorized `DATABASE_URL` for the existing diagnostic journal, and a new private manifest directory:
