...
51. Comparar cada preview v6 com a produção e registrar diferenças ainda desejadas. **Concluído em 2026-07-14** — matriz em `docs/plans/2026-07-14-phase7-preview-retirement.md`; zero diferenças pendentes.
...
58. Organizar schema e persistência por domínio sem criar abstrações genéricas com um único adapter. **Concluído em 2026-07-14** — sete tabelas e seis arquivos de tipos do loop de performance aposentado foram removidos; `client_output_learnings` foi agrupado com output learning; Brand Kit migrou para o diretório canônico de repositórios. A migração `0074` bloqueia tabelas não vazias e não usa `CASCADE`.

**Gate 7:** não há segunda árvore de produto e toda função visível sustenta a jornada central ou tem evidência de uso.

**Gate 7 — aprovado (2026-07-14):** árvore `/v6` ausente, superfícies congeladas invisíveis, loop de performance sem prova removido, inventário fresco e gate/build/suíte integral verdes. Evidência: `docs/plans/2026-07-14-gate7-evidence.md`.
