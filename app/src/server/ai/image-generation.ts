...
export async function generateAndStoreImage(
  ...
  if (tournamentEnabled && candidates.length === 0 && budget.remaining > 0) {
    providerRetries = 1;
    logImagePipelineStage({
  ...
  if (candidates.length === 0) {
    ...
    if (!tournamentEnabled && isRetryableProviderError(failures[0])) {
  ...
  const selection = tournamentEnabled && selectCandidate
    ...
    : 0;
  const winnerIndex = typeof selection === "number" ? selection : selection.winnerIndex;
  const selectionReason = typeof selection === "number" ? undefined : selection.reason;
