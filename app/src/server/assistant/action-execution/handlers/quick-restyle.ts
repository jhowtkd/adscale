...
/**
 * Assistente adapter for restyle — parse + error map only (item 33).
 * Domain resolution of baseCreativeId → campaign lives in restyleCampaign.
 */
export async function executeQuickRestyle(ctx: ActionExecutionContext) {
  ...
  const result = await restyleCampaign({
    ...
    userId: ctx.userId,
    locale: ctx.locale,
    baseCreativeId: parsed.data.baseCreativeId,
  ...
  if (!result.ok) {
    switch (result.error.code) {
      case "base_creative_not_found":
      case "missing_base_asset":
      case "invalid_base_asset":
        throw new AssistantActionExecutionError(
          result.error.code === "base_creative_not_found"
