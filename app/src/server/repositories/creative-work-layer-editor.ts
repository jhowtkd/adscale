...
function ownedLiveLease(input: Pick<LayerEditorMutationScope, "userId" | "leaseId">, now: Date) {
  return [
    sql`${creativeWorkOutputs.layerEditor}->'lease'->>'id' = ${input.leaseId}`,
    sql`${creativeWorkOutputs.layerEditor}->'lease'->>'userId' = ${input.userId}`,
    sql`(${creativeWorkOutputs.layerEditor}->'lease'->>'expiresAt')::timestamptz > ${now.toISOString()}::timestamptz`,
  ];
}

/**
 * Full-document editor mutations must retain the lease as it exists at write
 * time: heartbeat/release own that subtree and do not advance the revision.
 */
function withPersistedLease(next: LayerEditorStateV1) {
  return sql`jsonb_set(${JSON.stringify(next)}::jsonb, '{lease}', coalesce(${creativeWorkOutputs.layerEditor}->'lease', 'null'::jsonb))`;
}
...
export async function rollbackReservedLayerRegeneration(input: LayerEditorMutationScope & { operationId: string; now: Date }, executor: LayerEditorExecutor = db): Promise<Output | null> {
 const row=await getCreativeWorkLayerEditorOutput(input, executor); const state=layerEditorStateFromDatabase(row?.layerEditor);
 if(!state||state.revision!==input.expectedRevision||state.lease?.id!==input.leaseId||state.lease.userId!==input.userId||Date.parse(state.lease.expiresAt)<=input.now.getTime()||state.regeneration?.id!==input.operationId||state.regeneration.status!=="reserved")return null;
...
/** Refresh the durable pre-claim age after an event delivery is attempted. */
export async function refreshReservedLayerRegenerationDispatch(input: LayerEditorScope & { operationId: string; now: Date }, executor: LayerEditorExecutor = db): Promise<Output | null> {
 const row = await getCreativeWorkLayerEditorOutput(input, executor); const state = layerEditorStateFromDatabase(row?.layerEditor); const regeneration = state?.regeneration;
 if (!regeneration || regeneration.id !== input.operationId || regeneration.status !== "reserved") return null;
 const timestamp = input.now.toISOString();
 const layerEditor = sql`jsonb_set(jsonb_set(${creativeWorkOutputs.layerEditor}, '{regeneration,updatedAt}', ${JSON.stringify(timestamp)}::jsonb), '{updatedAt}', ${JSON.stringify(timestamp)}::jsonb)`;
 const [updated] = await executor.update(creativeWorkOutputs).set({ layerEditor, updatedAt: input.now }).where(and(scope(input), sql`${creativeWorkOutputs.layerEditor}->>'revision' = ${String(state.revision)}`, sql`${creativeWorkOutputs.layerEditor}->'regeneration'->>'id' = ${input.operationId}`, sql`${creativeWorkOutputs.layerEditor}->'regeneration'->>'status' = 'reserved'`)).returning();
 return updated ?? null;
}
