          ...
          <div className="rounded-[var(--radius-control)] bg-[var(--surface-inset)] p-3">
            ...
            {content ? (
              <>
                  ...
                  {contentFields.map(([key, label, value]) => (
                    <div key={key} data-testid={`variation-field-${key}`} data-origin={value ? "source" : "unknown"}>
                      ...
                      <dd className="font-medium text-[var(--text-primary)]">
                        {value || unknown}
                        <span className="ml-1.5 rounded-full bg-[var(--surface-raised)] px-1.5 py-0.5 text-[10px] font-normal text-[var(--text-muted)]">
                ...
                {content.entities?.length ? <div className="mt-3 flex flex-wrap gap-1.5" aria-label={t("identifiedEntities")}>{content.entities.map((entity) => <span key={entity} className="rounded-full bg-[var(--surface-raised)] px-2 py-1 text-xs text-[var(--text-secondary)]">{entity}</span>)}</div> : null}
          ...
          <div className="rounded-[var(--radius-control)] bg-[var(--surface-inset)] p-3">
            ...
            {style ? (
              <div className="mt-2 space-y-3 text-sm text-[var(--text-primary)]">
                ...
                {style.palette?.length ? <div><p className="text-xs text-[var(--text-muted)]">{t("dominantColors")}</p><div className="mt-1 flex flex-wrap gap-2" aria-label={t("paletteAria")}>{style.palette.map((sample) => <span key={`${sample.hex}-${sample.labelPt}`} className="inline-flex items-center gap-1.5 rounded-full bg-[var(--surface-raised)] px-2 py-1 text-xs"><span className="h-4 w-4 rounded-full border border-black/10" style={{ backgroundColor: sample.hex }} aria-hidden="true" />{sample.labelPt}</span>)}</div></div> : <p><span className="text-xs text-[var(--text-muted)]">{t("dominantColors")}: </span>{style.colorPalette.dominant.concat(style.colorPalette.accents).join(", ") || unknown}</p>}
                ...
                <div><p className="text-xs text-[var(--text-muted)]">{t("mood")}</p><div className="mt-1 flex flex-wrap gap-1.5" aria-label={t("moodAria")}>{(style.moodChipsPt?.length ? style.moodChipsPt : style.mood ? [style.mood] : []).map((chip) => <span key={chip} data-origin="inferred" className="rounded-full bg-[var(--surface-raised)] px-2 py-1 text-xs text-[var(--text-secondary)]">{chip}</span>)}</div></div>
                <div><p className="text-xs text-[var(--text-muted)]">{t("composition")}</p><div className="mt-1 flex items-center gap-2" aria-label={t("compositionDiagramAria")}><span className="grid h-10 w-14 grid-cols-3 gap-0.5 rounded border border-[var(--border-default)] bg-[var(--surface-raised)] p-1" aria-hidden="true"><span className="col-span-2 rounded-sm bg-[var(--neutral-bg)]" /><span className="rounded-sm bg-[var(--text-muted)]/30" /><span className="col-span-3 rounded-sm bg-[var(--text-muted)]/20" /></span><span>{style.compositionPt || style.composition || unknown}</span></div></div>
              </div>
            ) : <p className="mt-1 text-sm text-[var(--text-primary)]">{t("variationAnalysisUnavailable")}</p>}
          </div>
        </div>
