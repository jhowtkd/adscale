...
/**
 * Stored references must carry real bytes: a File, or a Blob preserved with
 * separate metadata ({ data, name, ... }) rebuilt into a File. Metadata-only
 * objects are rejected — they prove nothing about the bytes.
 */
function normalizeStoredFile(entry) {
  const BlobCtor = typeof Blob === 'function' ? Blob : null;
  const FileCtor = typeof File === 'function' ? File : null;
  const validMeta = (name, type, size) => typeof name === 'string' && name.length > 0
    && VALID_TYPES.has(type) && Number.isFinite(size) && size > 0 && size <= MAX_FILE_BYTES;
  if (FileCtor && entry instanceof FileCtor) {
    if (!validMeta(entry.name, entry.type, entry.size)) return null;
    return entry;
  }
  if (entry && typeof entry === 'object' && BlobCtor && entry.data instanceof BlobCtor && FileCtor) {
    const type = entry.type ?? entry.data.type;
    const size = Number.isFinite(entry.size) ? entry.size : entry.data.size;
    if (!validMeta(entry.name, type, size)) return null;
    try {
      return new FileCtor([entry.data], entry.name, { type, lastModified: entry.lastModified ?? Date.now() });
    } catch { return null; }
  }
  return null;
}
export function parseDraft(value, now = Date.now()) {
  ...
  for (const entry of value.files) {
    const file = normalizeStoredFile(entry);
    if (!file) return null;
    files.push(file);
  }
