...
describe("guest-controller", () => {
  ...
  it("attachments off: no attach button, honest guidance, programmatic select/drop/paste refused", () => {
    ...
    // Programmatic file-input change: no input exists while attachments
    // are off, so selection is refused by construction.
    expect(root.querySelector("#ag-file-input")).toBeNull();
