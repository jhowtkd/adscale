...
export function renderShell(assetBase = '/adscale-guest', resolveAsset, shellOptions = {}) {
  const attachmentsEnabled = shellOptions.attachmentsEnabled === true;
  const asset = (name) => escapeHtml(resolveAsset ? resolveAsset(name) : `${assetBase.replace(/\/$/, '')}/${name}`);
  const logo = (extraClass) => `<img src="${asset('logo.svg')}" alt="" width="130" height="23" class="${extraClass}"/>`;
  const attachGroup = attachmentsEnabled
    ...
    : `<div class="ag-attach-group"><input class="ag-sr" id="ag-file-input" type="file" accept="image/png,image/jpeg,image/webp" multiple tabindex="-1" aria-label="Referências desligadas" disabled hidden/><span class="ag-file-hint">Adicione referências depois, no Estúdio</span></div>`;
  const nav = [
  ...
  <aside class="ag-sidebar" aria-label="Navegação principal">
    <a class="ag-logo" href="/hi" aria-label="Adscale, início" data-action="focus">${logo('ag-logo-img')}</a>
