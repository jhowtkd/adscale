...
export async function POST(request: Request) {
  try {
    ...
    const [campaign, rawDerivations] = await Promise.all([
    ...
    const derivations = rawDerivations.map(toDerivationLike);
    ...
    const expandedIds = expandPackageDerivationIds(selectedRootIds, derivations);
