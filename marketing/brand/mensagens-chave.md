# Mensagens-chave — v2 (3 pilares Lab Notes)

> **3 mensagens canônicas, uma por pilar Lab Notes.**
> Substitui `v1` (5 mensagens) a partir de 2026-06-25.
> Veja ADR `0001-pilares-3-lab-notes.md` para a decisão completa.

---

## Pilar A — Lab Notes · Tese (40% do conteúdo)

> **Curator > operator.**
> A IA reposiciona o humano — sai da cadeira de operator (quem executa cada variação manualmente) e senta na cadeira de curator (quem define briefing, supervisiona geração, aprova/descarta e itera).

**Defesa:**
- A parte repetitiva da produção criativa (variação, adaptação de formato, retrabalho) some com IA.
- A parte estratégica (briefing, priorização de ângulo, decisão do que vai pro ar) fica com o humano — e fica **mais visível**.
- "IA mata o designer" é framing errado. O designer sai de operator pra curator — e roda mais.

**Quando usar:** posts que polemizam, comparam com concorrentes (AdCreative.ai, Pencil, Madgicx), ou defendem a worldview.

**Esqueleto:** Provocação → Argumento → Prova → CTA.

---

## Pilar B — Lab Notes · Experimento (40% do conteúdo)

> **Briefing estruturado + batch em vez de projeto + curadoria contínua = 10x criativos sem inflar headcount.**
> ADScale é uma cadeira de curator, não uma ferramenta de geração.

**Defesa:**
- Briefing estruturado (5 campos, 2 minutos) elimina retrabalho.
- Batch (1 base → dezenas de derivações) reduz custo unitário por variação.
- Curadoria humana no fim (mata ruim, refina bom, aprova vencedor) preserva qualidade.

**Quando usar:** tutoriais, demos de feature, bastidor técnico, "como fazer X em Y minutos", comparações com workflow manual.

**Esqueleto:** Premissa → Gargalo → Tese → Demo → Evidência → CTA.

---

## Pilar C — Lab Notes · Bastidor (20% do conteúdo)

> **Construir SaaS B2B em público, com números reais (positivos ou negativos), é o que gera confiança que copy não compra.**
> ADScale é build in public — não finge perfeição.

**Defesa:**
- Mostrar o que está sendo construído, o que quebrou, o que aprendeu.
- Decisões técnicas (escolha de modelo, arquitetura, tradeoff) viram posts.
- A trajetória do fundador (carreira, aprendizados de construir) entra na conta, sem virar influencer.

**Quando usar:** updates de produto, decisões técnicas compartilhadas, aprendizados genéricos de construir SaaS, milestones (positivos ou negativos).

**Esqueleto:** Essa semana → O que aprendi → CTA suave.

---

## O que mudou vs. v1 (5 mensagens)

| # v1 | v1 (deprecated 2026-06-23) | v2 (canônico 2026-06-25) |
|-----|----------------------------|---------------------------|
| 1 | Velocidade de teste é a nova vantagem competitiva | **Absorvida** em Pilar B (batch = mais velocidade). |
| 2 | IA substitui o operacional, não o estratégico | **Reframeada** como Pilar A (Curator > operator). |
| 3 | Ferramenta boa vira hábito; ferramenta ruim vira despesa | **Mantida como tese implícita** no Pilar B (cadeira de curator = hábito, não one-shot). |
| 4 | Agência que escala criativo escala margem | **Reframeada como Pilar B** (universalizada: "time que escala criativo escala margem"). |
| 5 | Construir em público > fingir perfeição | **Tornou-se Pilar C inteiro** (build in public). |

**Pilares cortados na migração v1 → v2:**
- ❌ Pilar "Casos e resultados" — absorvido em Pilar A (prova) e Pilar B (números).
- ❌ Pilar "Comunidade e conversa" — virou prática operacional (resposta a DMs/comentários), não conteúdo programático.

---

*Mantido em `marketing/brand/` · PT-BR (EN quando indicado) · Versão 2 · 2026-06-25 · Owner: Jhonatan Soares*
