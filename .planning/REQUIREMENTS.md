...
- [x] **COCK-01**: Owner vê evento `recipe_tradeoff_viewed` quando operador abre o painel de receita (F-08).
- [x] **COCK-02**: Owner vê evento `recipe_selected` com `recipeId` quando operador escolhe uma receita (F-09).
- [x] **COCK-03**: Owner vê funil de seleção de receita agregado por `recipeId` no dashboard.
- [x] **COCK-04**: Funil de preview não marca abandono falso ao revisar receita; `cockpit_stage_completed` reflete aprovação real (F-06).
- [x] **COCK-05**: Owner vê abandono do guided briefing quebrado por etapa (`stepId`) no analytics (F-12).

### Readiness (READY)

- [x] **READY-06**: Operador pode declarar override de falso positivo de readiness e continuar o fluxo com evento auditável (F-11).
- [x] **READY-07**: Owner vê sinais de override sem duplicar contagem entre nota de operador e evento.

### Analytics Owner (DASH)

- [x] **DASH-04**: Timeline de estágios de sessão sem cap artificial de 24 linhas (com janela de data padrão).
- [x] **DASH-05**: Owner vê funil de consumo de créditos por etapa (crédito → preview → aprovação).
- [x] **DASH-06**: Filtro de sessão no dashboard populado com sessões reais da API.
...
- [x] **QA-03**: Teste `creative-quality-gate-orchestration` alinhado ao formato atual (F-14).
- [x] **QA-04**: `npm test`, `npm run lint` e `npm run build` passam em `app/` após todas as mudanças.

## Traceability

| Requirement | Phase | Status |
|-------------|-------|--------|
| COCK-01 | Phase 85 | Complete |
| COCK-02 | Phase 85 | Complete |
| COCK-03 | Phase 85 | Complete |
| COCK-04 | Phase 85 | Complete |
| COCK-05 | Phase 85 | Complete |
| READY-06 | Phase 86 | Complete |
| READY-07 | Phase 86 | Complete |
| DASH-04 | Phase 87 | Complete |
| DASH-05 | Phase 87 | Complete |
| DASH-06 | Phase 87 | Complete |
| QA-03 | Phase 88 | Complete |
| QA-04 | Phase 88 | Complete |
