...
- [x] **CRED-03**: `credit_spend` / `credit_blocked` events include `operation_key` and estimate vs actual delta when applicable.
- [x] **CRED-04**: Preview gate shows credits already spent plus estimate disclaimer (extends v11.8 F-01).

### Entrega (DELIV)

- [x] **DELIV-01**: Approval package shows stale state with actionable tooltip/copy (F-10).
- [x] **DELIV-02**: Share link flow includes self-serve guidance without operator hand-holding (Q7).
- [x] **DELIV-03**: Approval package refresh communicates when assets are outdated.

### Analytics Owner (DASH)

- [x] **DASH-01**: Owner sees credit surprise ranking by operation on `OwnerAnalyticsPanel` (F-15).
- [x] **DASH-02**: Session timeline with stage gaps on owner dashboard (F-07).
- [x] **DASH-03**: CSV export includes credit surprise by operation columns.

### Verificação (QA)

- [x] **QA-01**: Regression tests for credit copy, batch gate, and stale package states.
- [x] **QA-02**: `npm test`, `npm run lint`, and `npm run build` pass in `app/`.
...
| CRED-03 | Phase 81 | Complete |
| DELIV-01 | Phase 82 | Complete |
| DELIV-02 | Phase 82 | Complete |
| DELIV-03 | Phase 82 | Complete |
| DASH-01 | Phase 83 | Complete |
| DASH-02 | Phase 83 | Complete |
| DASH-03 | Phase 83 | Complete |
| QA-01 | Phase 84 | Complete |
| QA-02 | Phase 84 | Complete |
