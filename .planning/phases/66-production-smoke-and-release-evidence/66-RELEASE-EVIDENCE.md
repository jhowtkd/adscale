...
| Field | Value |
|-------|-------|
| `origin/main` (last push) | `ec2f1a4` — review fixes (recipe billing, handoff) |
| Local HEAD (planning + SHIP-03 refactor) | `e4db288` + uncommitted hook refactor |
| Target deploy ref | Push SHIP-03 refactor commit before operator smoke |

## SHIP-03 — Post-review fixes and tests

| Fix | Status | Evidence |
|-----|--------|----------|
| Recipe selection sync on async readiness | ☑ | `use-strategy-recipe.ts` — `manualRecipeId ?? defaultRecipeId`, `resetKey` from `strategyRecipeSession` |
...
| Derivation flow tests (strategy_recipe entry) | ☑ | `use-derivation-flow.test.ts` updated |
...
| Post-deploy smoke ref updated | ☑ prep | ☐ pending | Update `65-SMOKE-EVIDENCE.md` git ref after deploy |
...
|-------------|------------------|
| SHIP-03 | **PASS** (pending commit/push of lint refactor) |
