...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Aceitar todos' cookie button (index 173) to accept cookies, then navigate to http://localhost:3000/library to check workspace library assets.
        ...
        # -> Sign in using dev-admin@adscale.local (input email and password) and click the Sign in button to access the app, then navigate to /library if needed.
        ...
        # -> Navigate to /library and load the workspace library page so assets can be verified.
        await page.goto("http://localhost:3000/library")
        try:
            await page.wait_for_load_state("domcontentloaded", timeout=5000)
        except Exception:
